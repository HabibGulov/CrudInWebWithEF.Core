...
public class PersonController : ControllerBase
{
    ...
    {
        IEnumerable<Person> people = await personRepository.GetUsersAsync();
        if (people == null)
        {
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<IResult> DeletePerson(IPersonRepository personRepository, [FromRoute] int? id)
    {
        if (id == null)
        {
            return Results.BadRequest("Id is null.");
        }
        bool res = await personRepository.DeletePersonAsync((int)id);
        if (res == false)
        {
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IResult> GetById(IPersonRepository personRepository, [FromRoute] int? id)
    {
        if (id == null)
        {
            return Results.BadRequest("Id is null.");
        }
        Person? person = await personRepository.GetPersonById((int)id);
        if (person == null)
        {
